...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def select(paraphrase):
        paraphrase = lower(paraphrase)
        paraphrase = remove_punctuation(paraphrase)
        paraphrase = split(paraphrase)
        for string in paraphrase:
            if string in topic:
                return True
        return False
    return select
    ...
    if typed == '' and reference == '':
        ...
        for i in range(len(typed_words)):
            if i >= len(reference_words):
                ...
                valid_count += 1
        return valid_count / len(typed_words) * 100
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    return (total_chars / 5) * (60 / elapsed)
    # END PROBLEM 4
