...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        start = self.place
        distance = 0
        while start != beehive and distance <= self.max_range:
            if distance < self.min_range:
                start = start.entrance
                distance += 1
            else:
                if bee_selector(start.bees) != None:
                    return bee_selector(start.bees)
                start = start.entrance
                distance += 1
        # END Problem 3 and 4
...
# The HungryAnt Class
class HungryAnt(Ant):
    """A bee once tried to test me,
    I ate his liver and a nice Chianti."""
    ...
    def action(self, gamestate):
        """Eat the random poor bee when the chewing is 0"""
        if self.chewing == 0:
...
class TankAnt(ContainerAnt):
    """Take my furious tongue of fire!!"""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, health=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not QueenAnt.has_real:
            self.is_real = True
        else:
            self.is_real = False
        QueenAnt.has_real = True
        super().__init__(health)
        # END Problem EC
        ...
        if self.is_real == False:
            ...
            while location != None:
                if location.ant and location.ant.is_container():
                    if location.ant.damage:
                        location.ant.buff()
                    if location.ant.contained_ant and location.ant.contained_ant.damage:
                        location.ant.contained_ant.buff()
                elif location.ant and location.ant.damage:
                    location.ant.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            self.slow_turn -= 1
            if self.health > 0:
                if self.scare_turn and gamestate.time % 2 == 0:
                    self.scare_turn -= 1
                    destination = self.place.entrance
                elif gamestate.time % 2:
                    destination = self.place
        elif self.scare_turn:
            self.scare_turn -= 1
            destination = self.place.entrance
        if not destination.is_hive() and destination is not None:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        ins_dis = {}
        while not start.is_hive():
            for bee in start.bees:
                ins_dis[bee] = distance
            if start.ant and start.ant is not self:
                ins_dis[start.ant] = distance
                if start.ant.is_container() and start.ant.contained_ant is not self:
                    ins_dis[start.ant.contained_ant] = distance
            distance += 1
            start = start.entrance
        return ins_dis
        # END Problem Optional 3
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        return max(self.damage - 0.25 * distance - 0.0625 * self.insects_shot, 0)
        # END Problem Optional 3
