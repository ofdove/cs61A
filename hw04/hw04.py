...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    subtrahend = interval(-1 * upper_bound(y), -1 * lower_bound(y))
    return add_interval(x, subtrahend)
...
def multiple_references_explanation():
    ...
    a formula to compute with intervals using Alyssa's system will
    produce tighter error bounds if it can be written in such a form
...
def quadratic(x, a, b, c):
    ...
    right = upper_bound(x)  
    left = lower_bound(x)
    right_y =  a * (right ** 2) + b * right + c
    left_y = a * (left ** 2) + b * left + c

    if right > vertex and left < vertex:
        vertex_y = a * (vertex ** 2) + b * vertex + c
        return interval(min(vertex_y, right_y, left_y), max(vertex_y, right_y, left_y))
    else:
        return interval(min(right_y, left_y), max(right_y, left_y))
